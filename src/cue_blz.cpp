...
unsigned char *LoadBlz(char *filename, unsigned int *length, int min, int max);
void  SaveBlz(char *filename, unsigned char *buffer, int length);
unsigned char *MemoryBlz(int length, int size);
...
unsigned short BLZ_CRC16(unsigned char *buffer, unsigned int length);

/*----------------------------------------------------------------------------*/
int Blz::decode(char* file) {
  // int cmd, mode;
  // int arg;
  ...
  // else if (!strcasecmp(argv[1], "-en9")) { cmd = CMD_ENCODE; mode = BLZ_NORMAL; }
  // else if (!strcasecmp(argv[1], "-eo9")) { cmd = CMD_ENCODE; mode = BLZ_BEST; }
...
void BLZ_Decode(char *filename) {
  ...
  raw_buffer = (unsigned char *) MemoryBlz(raw_len, sizeof(char));
  ...
  SaveBlz(filename, raw_buffer, raw_len);
...
void BLZ_Encode(char *filename, int mode) {
  ...
  raw_buffer = LoadBlz(filename, &raw_len, RAW_MINIM, RAW_MAXIM);
  ...
  SaveBlz(filename, pak_buffer, pak_len);
...
#define SEARCH(l,p) { \
  ...
  pak_buffer = (unsigned char *) MemoryBlz(pak_len, sizeof(char));
  ...
  if (!pak_tmp || (raw_len + 4 < ((pak_tmp + raw_tmp + 3) & -4) + 8)) {
    ...
    *(unsigned int *)pak = 0; pak += 4;
    ...
    tmp = (unsigned char *) MemoryBlz(raw_tmp + pak_tmp + 11, sizeof(char));
